...
class LoginPage(QtWidgets.QMainWindow):
    ...
    def on_confirm_button_click(self):
        account = self.ui.Account_TextEdit.text()
        password = self.ui.Password_TextEdit.text()
        authenticator = ldap_authenticator.LDAPAuthenticator()
        if self.ui.Remember_me_checkbox.isChecked():
            with open(FILE_NAME, "w") as json_file:
                json_file.write(json.dumps({"account": account}))
        if account != "" and password != "":
            if authenticator.whitelist_auth("auto_tt_whitelist", account, password):
                ...
                self.ui.Hint_Label.setStyleSheet("color:Red")
                self.ui.Hint_Label.setText("Login failed, please check your credential.")
        else:
            self.ui.Hint_Label.setText("Please fill all info to log in.")
