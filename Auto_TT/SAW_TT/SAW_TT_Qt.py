...
try:
    from Auto_TT.modules import log_combiner, DrawLineChart, logcatParserV3, parser_and_create_issue, \
        ftp_tool, QNX_logger, usb_relay_control
except (ModuleNotFoundError, ImportError):
...
def resource_path():
    if getattr(sys, 'frozen', False):
        base_path = sys._MEIPASS + os.sep + "SAW_TT"
    else:
...
LOG_SAW = ABS_DIR_PATH + "Log_SAW" + os.sep
LOG_DATE = LOG_SAW + time.strftime("%Y_%m_%d", time.localtime()) + os.sep
LOG_SAW_LOGCAT = LOG_DATE + "logcat" + os.sep
LOG_SAW_BUGREPORT = LOG_DATE + "bugreport" + os.sep
LOG_SAW_QA_LOG = LOG_DATE + "qalog" + os.sep
...
SERVER_DIRECTION = r"\\MD-TEST\Projects\SAW\Monkey_log"
LOCAL_DIRECTION = r"D:\Projects\SAW\Monkey_log"
...
def create_log_folder_at_local():
    ...
    os.makedirs(LOG_SAW_LOGCAT, exist_ok=True)
    os.makedirs(LOG_SAW_BUGREPORT, exist_ok=True)
    os.makedirs(LOG_SAW_QA_LOG, exist_ok=True)
    os.makedirs(LOG_TIME_DIR, exist_ok=True)


class SAWui(Base_TT_Qt.BaseUI):
    def __init__(self):
        super().__init__()
        self.qnx_log_data = None
        self.qnx_logger = None
        _translate = QtCore.QCoreApplication.translate
        self.base_form.setWindowTitle(_translate("base_form", "SAW Test Tool"))
        ...
        self.base_form.setStyleSheet('#base_form{background-color: #77DEE6;}')
    ...
    def stop_log(self):
        ...
        logcat_file = LOG_SAW_LOGCAT + f"{LOG_TIME}-logcat.txt"
        subprocess.call(
    ...
    def generate_bugreport(self):
        ...
        subprocess.run(self.adb_command + ["bugreport", LOG_SAW_BUGREPORT])
    ...
    def pull_and_delete_bugreport(self, file_issue):
        ...
        bugreport_dir = LOG_SAW_BUGREPORT + f"BR_{LOG_TIME}"
    ...
    def upload_files(self):
        ...
        if getattr(sys, 'frozen', False):
            ...
            for directory in os.listdir(ABS_DIR_PATH):
                if directory in ["Log_SAW"] + PROJECTS:
        ...
        for directory in os.listdir(ABS_DIR_PATH):
            if directory == "Log_SAW":
                self.upload_dirs_to_ftp(ftp_ip, ftp_account, ftp_password, os.path.abspath(LOG_SAW),
                                        remote_path_add + "Monkey_log")
            elif directory in PROJECTS:
                for log_dir in os.listdir(directory):
                    self.upload_dirs_to_ftp(ftp_ip, ftp_account, ftp_password,
    ...
    def get_test_log(self, is_monkey_test=True):
        ...
        log_saw_qalog_time_ = LOG_SAW_QA_LOG + LOG_TIME + os.sep
        os.makedirs(log_saw_qalog_time_, exist_ok=True)
        for system in MONITOR_SYSTEM_LIST:
            if system != "DISK":
                for item in get_package_data(config_test_package_list):
                    self.pull_remove_file(
                        f"/data/local/tmp/qa/qa_{item}_{system}.csv", log_saw_qalog_time_
                    )
                    # print(Path(log_saw_qalog_time_ + f"qa_{item}_{system}.csv"))
                    DrawLineChart.draw_line_chart(
                        str(Path(log_saw_qalog_time_ + f"qa_{item}_{system}.csv")), system, 2)

            self.pull_remove_file(f"/data/local/tmp/qa/qa_{system}_monitor.csv",
                                  log_saw_qalog_time_)

            DrawLineChart.draw_line_chart(
                str(Path(f"{log_saw_qalog_time_}qa_{system}_monitor.csv")), system)
        if is_monkey_test:
    ...
    def monitor_thread(self, system):
        ...
        subprocess.call(
            self.adb_command + ["push", Path(f"{ABS_DIR_PATH}/shell/qa_restart{system}.SAW.sh"),
                                f"{DUT_LOG_DIRECTORY}qa_restart{system}.SAW.sh"]
        )
        # grant execute permission
        subprocess.call(
        ...
        # trigger qa_restart{system}.SAW.sh in another process
        subprocess.call(self.adb_command + ["shell",
                                            f"cd /data/local/tmp/qa && nohup ./qa_restart{system}.SAW.sh &"],
                        timeout=10)
