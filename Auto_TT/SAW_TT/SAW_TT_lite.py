...
def get_execution_time(action):
    ...
    if action == "start":
        ...
        with open(SAW_TT_Qt.LOG_DATE + "outputFolder" + os.sep + SAW_TT_Qt.LOG_TIME + os.sep + "Execution_time.txt",
                  "w") as fo:
...
def get_config_data():
    ...
    with open(SAW_TT_Qt.config_common) as j:
        data = json.load(j)
...
def check_device_status(ui):
    ...
    if "10.1.1.3" not in str(adb_devices_output) or str(adb_devices_output) == r"b'List of devices attached\n\n'":
        print("No device found")
        try:
            connect_to_adb()
        except Exception:
            print("failed to trigger relay, " + traceback.format_exc())
...
def send_mail(recipients_list, cc_list, file_dir_path):
    ...
    recipients_list += set(mail_tool.fetch_mail_group("saw_mail_group",
                                                      "where position in ('PM', 'SM', 'Tech Leader') "
                                                      "or position like '%PO%'"))
    cc_list += set(mail_tool.fetch_mail_group("saw_mail_group", "where position like '%QA%'"))
    mail = mail_tool.Mail(
        title=f"[SAW] Monkey_Test_Result_{date}",
        body=mail_body,
...
def create_mail_body(file_dir_path):
    ...
    server_log_dir = SAW_TT_Qt.SERVER_DIRECTION
    body = ('\n'
            '<html>\n'
            '<body>\n'
            '<p>Hi, all</p>\n'
...
def connect_to_adb():
    trigger_usb_relay()
    time.sleep(20)
    print("adb connect 10.1.1.3")
    adb_connect_output = subprocess.check_output(["adb", "connect", "10.1.1.3"])
    print(adb_connect_output)
    if "failed to connect" in str(adb_connect_output):
        print("Failed to connect to adb")


def trigger_usb_relay():
    from modules.usb_relay_control import usb_relay_controller
    usb_relay = usb_relay_controller.UsbRelay("COM")
    usb_relay.close(1)
    time.sleep(1)
    usb_relay.open(1)
...
if __name__ == '__main__':
    ...
    get_config_data()
    app = SAW_TT_Qt.QtWidgets.QApplication(sys.argv)
    ...
    with open(SAW_TT_Qt.config_test_package_list, 'w') as file:
    ...
    SAW_TT_Qt.LOG_DATE = SAW_TT_Qt.LOG_SAW + time.strftime("%Y_%m_%d", time.localtime()) + "_" + str(
        test_data_dir) + os.sep
    SAW_TT_Qt.LOG_SAW_LOGCAT = SAW_TT_Qt.LOG_DATE + "logcat" + os.sep
    SAW_TT_Qt.LOG_SAW_BUGREPORT = SAW_TT_Qt.LOG_DATE + "bugreport" + os.sep
    SAW_TT_Qt.LOG_SAW_QA_LOG = SAW_TT_Qt.LOG_DATE + "qalog" + os.sep
    SAW_TT_Qt.LOG_TIME = time.strftime(
        "%Y_%m_%d-%H_%M_%S", time.localtime()
    )
    SAW_TT_Qt.LOG_TIME_DIR = SAW_TT_Qt.LOG_DATE + os.sep + "outputFolder" + os.sep + SAW_TT_Qt.LOG_TIME + os.sep
    SAW_TT_Qt.create_log_folder_at_local()
    # write test packages data
    with open(SAW_TT_Qt.LOG_TIME_DIR + "Test_packages.txt", 'w') as file:
    ...
    SAW_TT_Qt.log_combiner.combine_log(SAW_TT_Qt.LOG_TIME_DIR)
    # upload logs to FTP
    upload_thread = threading.Thread(target=UI.upload_files)
    upload_thread.start()
    # Send report
    if is_mail_report:
        send_mail(monkey_config["recipients_list"], monkey_config["cc_list"], SAW_TT_Qt.LOG_TIME_DIR)
