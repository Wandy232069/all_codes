...
class SAWui(Base_TT.UIPage):
    def __init__(self):
        super().__init__()
        main_page = super().main_page
        super().main_page.title(
            "  SAW_TestTool                                         Contributed by AutoTeam")
        main_page.configure()
        ttk_style = ttk.Style()
    ...
    def monitor_thread(self, system):
        ...
        # print(f"adb push {ABS_DIR_PATH}/shell/qa_restart{system}.SAW.sh " +
        ...
        # grant execute permission
        # print(
        #     f"adb shell chmod +x {DUT_LOG_DIRECTORY}qa_restart{system}.SAW.sh \n")
        subprocess.call(
    ...
    def upload_files(self):
        ...
        for directory in os.listdir(ABS_DIR_PATH):
            if directory == "Log_SAW":
                upload_dirs_to_ftp(ftp_ip, ftp_account, ftp_password, os.path.abspath(LOG_SAW),
                                   remote_path_add + "Monkey_log")
            elif directory in PROJECTS:
                for log_dir in os.listdir(directory):
                    upload_dirs_to_ftp(ftp_ip, ftp_account, ftp_password,
